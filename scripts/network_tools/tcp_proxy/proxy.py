...
def receive_from(connection):
    buffer = b""
    connection.settimeout(5) # increase timeout if necessary
    try:
        while True:
...
def main():
    if len(sys.argv[1:]) != 5:
        print("Usage: ./proxy.py [localhost] [localprt]", end='')
        print("[remotehost] [remoteport] [receive_first]")
        print("Example: ./proxy.py 127.0.0.1 900 10.12.132.1 900 True")
        sys.exit(0)
